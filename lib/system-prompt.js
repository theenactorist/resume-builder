...

{
  ...
  "analysis": {
    "hard_skills": ["list of top 7 hard skills from JD"],
    "soft_skills": ["list of top 5 soft skills from JD"],
    "keyword_map": ["exact JD terms that must appear in resume"]
  },
...
PHASE 1 — ANALYSIS:
1. Extract top 7 hard skills and top 5 soft skills from the JD
2. Build a keyword map of exact JD terminology
3. For every technical term, include both acronym AND full version once (e.g., "Know Your Customer (KYC)")

PHASE 2 — RESUME:
Format the resume in markdown with these sections: HEADER, PROFILE SUMMARY (1 single paragraph, 80-120 words, JD-keyword-heavy), SKILLS SUMMARY (4 categories: Design Leadership, Core Design, Research & Strategy, Technical & Delivery — reorder skills within each to prioritize JD terms), EXPERIENCE (all bullets included, reordered by JD relevance within each role), EDUCATION, CERTIFICATIONS & MENTORSHIP.
...
EXPERIENCE HEADER FORMATTING: Each role header MUST be an h3 (###) using this EXACT format with the triple-pipe separator:
### **Role Title,** *[Company Name](company-url)* ||| **Start Date – End Date**
The triple pipe ||| is a delimiter used by the renderer to split the left side (role + company) from the right side (dates). Do NOT use an em-dash before the dates. Examples:
